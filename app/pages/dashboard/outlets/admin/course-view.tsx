import { useStudentsFromCourse } from '@/features/edu/hooks/queries'
...
import { RefetchButton } from '@/components/other/refetch-button'
    ...
    { accessorKey: "attendance", header: "Events Attended" },
    {
        ...
        header({ column }) {
            return <p className='inline-flex items-center gap-1'>
                <SortableColumn column='missing' label='Missing Surveys' />
                <span
                    className='relative'
                    title={`
...
export default function CourseView() {
    const { courseId } = useAppNavigation()

    const { data, isLoading } = useStudentsFromCourse(courseId)
