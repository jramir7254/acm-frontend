...
export function FilteredColumn({ column, children, label }: { column: string, children: React.ReactNode, label?: string }) {
    const table = useTableContext()
    if (!table) return
    ...
    return (
        <div className='inline-flex items-center gap-1'>
            <span>{label || column.charAt(0).toUpperCase() + column.slice(1)}</span>
            <Popover>
...
export function SortableColumn({ column, label }: { column: string, label?: string }) {
    const table = useTableContext()
    if (!table) return
